...
describe('runCommand', () => {
  ...
  it('resolves when a command writes to stdout and exits 0', async () => {
    ...
    await expect(runCommand('node', ['-e', 'console.log("ok")'])).resolves.toBeUndefined();
  ...
  it('rejects when the command cannot be started', async () => {
    ...
    await expect(runCommand(cmd)).rejects.toThrow(new RegExp(`Failed to start command "${cmd}"`));
  });
});
